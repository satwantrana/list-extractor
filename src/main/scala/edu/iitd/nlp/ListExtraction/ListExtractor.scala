...
case class ListRange(ccPos: Int, elemsRange: mutable.ArrayBuffer[(Int, Int)], var conf: Double, var defaultIndices: Params = Params())
...
trait ListExtractor extends LoggingWithUncaughtExceptions {
  ...
  def extractLists(tokens: Seq[PostaggedToken], lists: Seq[ListRange]): Seq[List] = {
    lists.map {
      case ListRange(ccId, elems, conf, defaultIndices) => List(
